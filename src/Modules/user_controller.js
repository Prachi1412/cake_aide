		...
		.then(result => {
			if(mobile_number.length <10){
				...
				for(let i=0; i< req.files.length ;i++) {
					...
					let updateData = {business_name , mobile_number ,address , profile_image , is_profile_created : 1};
...
exports.forgotPassword = (req , res) => {
	...
	.then(result => {
		...
		.then(userResult =>{
			...
			let condition = {user_id};
			UserModel.updateQuery(updateData , condition)
			.then((userResponse) => { responses.successOtp(res , constant.responseMessages.OTP_SENT)
